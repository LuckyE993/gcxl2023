...
def Wifi_Scan_Start():
    ...
    for ap in access_points:
        ssid = ap['ssid']
        if re.match(r'(\d{3})-(\d{3})', ssid):  # 匹配 "132-321" 格式的SSID
            digits = re.sub(r'[^\d]', '', ssid)  # 仅保留数字部分
            for i in range(6):
                parameter.WiFi_Scan.task_number[i] = int(digits[i])
            print(parameter.WiFi_Scan.task_number)

def Wifi_Scanner_thread(): 
    while True:
        if parameter.Mode.task_detect is 7:
            Wifi_Scan_Start()
            time.sleep(1)